...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QGroupBox, QSplitter, QListWidget,
    QCheckBox, QSpinBox, QDoubleSpinBox, QProgressBar, QStatusBar,
    QMenuBar, QMenu, QToolBar, QDockWidget, QListWidgetItem, QComboBox
)
...
from src.backend.translation_backend import TranslationManager, ModelLoadThread, TranslationThread, ModelDownloadThread
...
class TranslatorMainWindow(QMainWindow):
    ...
    def load_model(self):
        ...
        self.load_thread = self.manager.load_model(
            model_path,
            self.update_status,
    ...
    def on_download_complete(self, success, model_path, progress_dialog):
        ...
        if success:
            ...
            QMessageBox.critical(self, "Download Failed", "Failed to download the model. Please check your internet connection and try again.")
    ...
    def load_settings(self):
        ...
        if last_model and os.path.exists(last_model):
            ...
            if self.backend_combo.currentIndex() == 0 and last_model.lower().endswith(".gguf") and "bf16" in os.path.basename(last_model).lower():
                self.model_path_label.setText("No model loaded")
                self.load_button.setEnabled(False)
            else:
